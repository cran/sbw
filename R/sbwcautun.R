.sbwcautun = function(dat, ind, out, bal, wei, sol, par) {
  ...
  if (par$par_est %in% c("ate", "cate")) {
    sbwtun_level = lapply(dat_level, .sbwauxtun, bal = bal, wei = wei, sol = sol, run = FALSE)
    ...
    if (par$par_est %in% "att") {
      sbwfix_level = .sbwauxtun(dat_level[[1]], bal = bal, wei = wei, sol = sol)
      # Get weights
      ...
      sbwfix_level = .sbwauxtun(dat_level[[2]], bal = bal, wei = wei, sol = sol)
      # Get weights
      weights = sbwfix_level$dat_weights$weights
      # Calculate effective sample size
      eff_size = sum(weights)^2/sum(weights^2)
