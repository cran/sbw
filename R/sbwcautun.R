.sbwcautun = function(dat, ind, out, bal, wei, sol, par) {
  ...
  if (par$par_est %in% c("ate", "cate")) {
    ...
    sbwtun_level = lapply(dat_level, .sbwauxtun, bal = bal, wei = wei, sol = sol, sd_target = sd_target, run = FALSE)
    cstat = colSums(do.call(rbind,lapply(sbwtun_level, function(x) x$cstat)))
    bal$bal_tol = bal$bal_gri[which.min(cstat)]
    ...
    bal$bal_tol = sbwfix_level[[1]]$bal$bal_tol
    ...
    if (par$par_est %in% "att") {
      ...
      sbwfix_level = .sbwauxtun(dat_level[[1]], bal = bal, wei = wei, sol = sol, sd_target = sd_target)
      # Get weights
      ...
      sbwfix_level = .sbwauxtun(dat_level[[2]], bal = bal, wei = wei, sol = sol, sd_target = sd_target)
      # Get weights
      weights = sbwfix_level$dat_weights$sbw_weights
      # Calculate effective sample size
      effective_sample_size = sum(weights)^2/sum(weights^2)
    ...
    # Update the outputs
    bal$bal_tol = sbwfix_level$bal$bal_tol
    cstat = sbwfix_level$cstat
  ...
  output = list(ind = ind, out = out, bal = bal, cstat = cstat, objective_value = objective_value, effective_sample_size = effective_sample_size, time = time, status = status, dat_weights = dat_weights, shadow_price = shadow_price, balance_parameters = balance_parameters, par = par)
  return(output)
}